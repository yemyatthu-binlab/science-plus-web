...
type Props = {
  languageModelType?: string[];
};

const Terminal = ({ languageModelType }: Props) => {
  const [word, setWord] = useState("");
  const timeoutRef = useRef<NodeJS.Timeout>();
  const [randomIndex, setRandomIdx] = useState<number>();
  const [isSearching, setIsSearching] = useState(false);
  const [isError, setIsError] = useState(false);
  const [activeLangugeModel, setActiveLanguageModel] = useState(0);
  const [chat, setChat] = useState<SetStateAction<ChatSession>>();
  ...
  const handleClick = () => {
    languageModelType ? generativeTextFromTrainedModel() : generateMockText();
  };
  ...
  const generativeTextFromTrainedModel = async () => {
    console.log("bbb", chat);
  ...
  return (
    <div className="bg-black mt-14 rounded-lg">
      ...
      >
        {languageModelType ? (
          <div className="ml-4">
            {languageModelType.map((item, idx) => {
              return (
                <button
                  className={cn(
                    "bg-black p-1 px-2",
                    activeLangugeModel == idx
                      ? "border-green-500"
                      : "border-white"
                  )}
                  key={idx}
                  onClick={() => {
                    setActiveLanguageModel(idx);
                  }}
                >
                  <ThemeText
                    className={cn(
      ...
      <div className="relative">
        ...
        {languageModelType && languageModelType.length > 0 ? (
          <></>
        ) : (
          <>
